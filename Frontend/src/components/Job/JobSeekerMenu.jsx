...
import { 
  FaBars, FaTimes, FaTachometerAlt, FaUser, 
  FaBriefcase, FaRobot, FaClipboardList, FaSignOutAlt 
} from 'react-icons/fa';
...
const JobSeekerMenu = () => {
  ...
  const isSeekerLoggedIn = !!localStorage.getItem('email');
  if (!isSeekerLoggedIn) return null;
  ...
  const navItems = [
    { label: 'Dashboard', path: '/job', icon: <FaTachometerAlt /> },
    { label: 'My Profile', path: '/profile', icon: <FaUser /> },
    { label: 'AI Resume Studio', path: '/resume-studio', icon: <FaRobot /> },
    { label: 'Search Jobs', path: '/all-jobs', icon: <FaBriefcase /> },
    { label: 'My Applications', path: '/ap', icon: <FaClipboardList /> }
  ];
            ...
            >
              <div className="p-8 flex justify-between items-center border-b border-slate-200 dark:border-white/10">
                <h2 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight flex items-center gap-3">
                  <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-indigo-500 to-purple-600"></div> Menu
                </h2>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar">
                {navItems.map((item, idx) => (
                  <button
                    ...
                    className="w-full flex items-center gap-4 p-4 rounded-2xl text-slate-700 dark:text-slate-300 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 hover:text-indigo-600 dark:hover:text-indigo-400 font-bold transition-all group"
                  >
                    <div className="p-4 bg-slate-100 dark:bg-slate-800 rounded-xl shadow-sm border border-transparent group-hover:bg-indigo-600 group-hover:text-white transition-colors">
                      {item.icon}
                    </div>
        ...
        <div className="bg-white/80 dark:bg-slate-950/80 backdrop-blur-3xl border-t border-slate-200 dark:border-white/10 rounded-t-[2rem] shadow-[0_-10px_40px_rgba(0,0,0,0.1)] px-6 py-4 flex items-center justify-between">
          {navItems.slice(0, 4).map((item, idx) => (
            <button 
              ...
              className="flex flex-col items-center gap-1.5 p-2 text-slate-500 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors"
            >
